...
public class UserDeletionListener {
    ...
    public void handleDeletionEvent(UserDeletionEvent event) {
        log.info("Received UserDeletionEvent in workout-service: userId={}, type={}", event.getUserId(), event.getType());
        ...
        try {
            if (event.getType() == UserDeletionEvent.Type.START) {
                ...
                log.info("Rolling back deletion for userId={} in workout-service", event.getUserId());
            }
        } catch (Exception e) {
